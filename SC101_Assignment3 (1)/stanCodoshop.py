...
def get_best_pixel(pixels):
    ...
    for pixel in pixels:
        if get_pixel_dist(pixel, red, green, blue) < best_dist:
            best_dist = get_pixel_dist(pixel, red, green, blue)
            best = pixel
...
def solve(images):
    ...
    for x in range(width):
        for y in range(height):  # 總共x*y個像素點
            pixels = []
            for image in images:
                pixels.append(image.get_pixel(x, y))  # 取出每張圖片在(x, y)上的像素點 包成串列pixels
            result.set_pixel(x, y, get_best_pixel(pixels))  # results的每個像素點在位置(x, y)上是images張圖片在(x, y)的最佳像素點
